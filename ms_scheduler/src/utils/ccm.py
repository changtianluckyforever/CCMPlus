...
class ccm:
    ...
    def get_nearest_distances(self, t, t_steps, dists):
        ...
        nearest_inds = np.argsort(dist_t)[
                       1:self.E + 1 + 1]  # get indices sorted, we exclude 0 which is distance from itself
    ...
    def predict(self, t):
        ...
        # get weights
        u = np.exp(
            -nearest_distances / np.max([eps, nearest_distances[0]]))  # we divide by the closest distance to scale
        w = u / np.sum(u)
